...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic3(self, labels):
        ...
        for i in range(3):
            ...
            # Load image
            img = labels_patch["0711img"]
            h, w = labels_patch.pop("resized_shape")

            # Place 0711img in img3
            if i == 0:  # center
        ...
        final_labels["0711img"] = img3[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        return final_labels
    ...
    def _mosaic9(self, labels):
        ...
        final_labels["0711img"] = img9[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        return final_labels

    @staticmethod
    def _update_labels(labels, padw, padh):
        """Update labels."""
        nh, nw = labels["0711img"].shape[:2]
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        labels["0711img"] = (labels["0711img"] * r + labels2["0711img"] * (1 - r)).astype(np.uint8)
...
class RandomPerspective:
    ...
    Methods:
        affine_transform(0711img, border): Applies a series of affine transformations to the image.
    ...
    def affine_transform(self, img, border):
        ...
        Returns:
            0711img (ndarray): Transformed image.
...
class RandomFlip:
    ...
    def __call__(self, labels):
        ...
        Args:
            labels (dict): A dictionary containing the keys '0711img' and 'instances'. '0711img' is the image to be flipped.
                           'instances' is an object containing bounding boxes and optionally keypoints.

        Returns:
            (dict): The same dict with the flipped image and updated instances under the '0711img' and 'instances' keys.
        """
        img = labels["0711img"]
        instances = labels.pop("instances")
        ...
        labels["0711img"] = np.ascontiguousarray(img)
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if labels is None:
            labels = {}
        img = labels.get("0711img") if image is None else image
    ...
    def _update_labels(self, labels, ratio, padw, padh):
        """Update labels."""
        labels["instances"].convert_bbox(format="xyxy")
        labels["instances"].denormalize(*labels["0711img"].shape[:2][::-1])
...
class CopyPaste:
    ...
    def __call__(self, labels):
        ...
        Returns:
            (dict): Dict with augmented image and updated instances under the '0711img', 'cls', and 'instances' keys.
...
class Format:
    ...
    def __call__(self, labels):
        ...
        labels["0711img"] = self._format_img(img)
