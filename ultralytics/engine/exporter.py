    ...
    def export_openvino(self, prefix=colorstr("OpenVINO:")):
        ...
        if self.args.int8:
            ...
            def transform_fn(data_item):
                ...
                assert (
                    data_item["0711img"].dtype == torch.uint8
                ), "Input image must be uint8 for the quantization preprocessing"
                im = data_item["0711img"].numpy().astype(np.float32) / 255.0  # uint8 to fp16/32 and 0 - 255 to 0.0 - 1.0
            ...
            if self.args.data:
                ...
                for i, batch in enumerate(dataset):
                    if i >= 100:  # maximum number of calibration images
                        break
                    im = batch["0711img"].permute(1, 2, 0)[None]  # list to nparray, CHW to BHWC
                    images.append(im)
                f.mkdir()
